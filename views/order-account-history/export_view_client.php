...
$orderAccount = OrderAccountHistory::find()->andWhere(['client_id' => $client_id->id])->andWhere(['cr_date' => $cr_date])->orderBy(['id' => SORT_DESC])->one();
...
// $orderAccountSumm = OrderAccountHistory::find()->andWhere(['client_id' => $client_id->id])->andWhere(['cr_date' => $cr_date])->all();
$orderAccountSumm = OrderAccountHistory::find()
    ->where(['client_id' => $client_id->id])
    ->andWhere(['cr_date' => $cr_date])
    ->andWhere(['or',
        ['!=', 'is_worker', 1],
        ['is', 'is_worker', null]
    ])
...
foreach ($orderAccountSumm as $orderAccountValue) {
    $all_product_sum = $all_product_sum + $orderAccountValue->all_product_sum;
    $sum_dollar = $sum_dollar + $orderAccountValue->sum_dollar;
    $all_summ_dollar = $all_summ_dollar + $orderAccountValue->all_summ_dollar;
    $sum_som = $sum_som + $orderAccountValue->sum_som;
        ...
        <div class="panel panel-inverse" data-sortable-id="table-basic-4">
            ...
            <div class="panel-heading">
                <div class="panel-heading-btn">
                        <?php $form = ActiveForm::begin(['action' => '/order-account-history/client-product-history','options' => ['style' => 'display: inline;'],]);?>
                                <input type="hidden" name="customer_fio" value="<?= $customer_fio ?>" />
                    ...
                    <a href="<?= Url::to(['/order-account-history/print-history-client', 'id' => $order_id]) ?>" target ="_blank" class="btn btn-xs  btn-warning"> <i class="fa fa-download"></i> Chop qilish Mijoz uchun</a>
                    <?php }?>
                    ...
                    <?php if(Yii::$app->user->identity->permission == 1 || \Yii::$app->user->identity->permission == 2){?>
                        <table class="table">   
                            ...
                            <tr>
                                ...
                                <td ><b style="color:#474ba0"><?= Yii::$app->formatter->asDecimal($sum_dollar,2) ?>  $</b></td>
                            </tr>
                            <tr>
                                ...
                                <td  ><b style="color:#474ba0"><?= Yii::$app->formatter->asDecimal($sum_som, 2) ?></b></td>
                            </tr>
                            <tr>
                                <th nowrap style="text-align: left; width: 200px; color:#f59c1a">Chegirma ($):</th>
                                <td  ><b style="color:#f59c1a"><?= $discount_amount?>  $</b></td>
                                <td style="width: 10px;"></td>
                                <th nowrap style="text-align: left; color:#474ba0">To'langan summa kartada:</th>
                                <td ><b style="color:#474ba0"><?= Yii::$app->formatter->asDecimal($sum_cart, 2)?></b></td>
                            </tr>
                            <tr>
                                <th nowrap style="text-align: left; width: 150px;color:#f59c1a">Qolgan qarz ($)<?= $client_id->id?>:</th>
