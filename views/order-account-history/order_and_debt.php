            ...
            <div class="panel-body" style="background-color:rgb(210, 226, 245);">
                ...
                <div class="row row-space-12">
                    <?php $form = ActiveForm::begin([
                        ...
                        'options' => [
                            'class' => 'form-inline',
                            'style' => 'margin-bottom: 20px; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: flex-end;'
                        ]
                    ]); ?>
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                foreach ($grouped as $date => $groupRows):
                                    ...
                                    $sum_zdacha_sum_debt = 0;
                                    $sum_zdacha_dollar_debt = 0;
                                ?>
                                    <tr>
                                    ...
                                    <?php foreach ($groupRows as $row): ?>
                                        <tr>
                                            ...
                                            <td style="font-weight:bold;font-size:14px">
                                                <b><?= Html::encode($row['client_name']) ?></b>
                                            </td>
                                            <td style="font-weight:bold;font-size:14px">
                                        ...
                                        <?php
                                        $sum_all_product += (float)($row['all_product_sum'] ?? 0);

                                        $sum_paid += (float)($row['all_summ_dollar'] ?? 0);
                                        $sum_paid_som += (float)($row['sum_som'] ?? 0);
                                        $sum_paid_cart += (float)($row['sum_cart'] ?? 0);
                                        $sum_paid_transfers += (float)($row['sum_transfers'] ?? 0);
                                        $sum_paid_otkazma += (float)($row['sum_otkazma'] ?? 0);

                                        $sum_paid_debt += (float)($row['paid_debt'] ?? 0);
                                        $sum_paid_debt_som += (float)($row['debt_sum_som'] ?? 0);
                                        $sum_paid_debt_cart += (float)($row['debt_summ_cart'] ?? 0);
                                        ...
                                        if ($row['action'] === 'Buyurtma qilgan') {
                                            $sum_zdacha_sum_order += (float)($row['zdacha_sum'] ?? 0);
                                            $sum_zdacha_dollar_order += (float)($row['zdacha_dollar'] ?? 0);
                                        }

                                        if ($row['action'] === 'Qarz to‘lagan') {
                                            $sum_zdacha_sum_debt += (float)($row['zdacha_sum'] ?? 0);
                                            $sum_zdacha_dollar_debt += (float)($row['zdacha_dollar'] ?? 0);
                                        }
                                        ?>
                                    <?php endforeach; ?>
                                    ...
                                    <tr style="color:#348fe2; font-weight:bold;font-size:14px">
                                        ...
                                        <td style="font-weight:bold; font-size:14px; min-width:220px;">
                                        ...
                                        <td style="font-weight:bold; font-size:15px; min-width:220px;">
                                            <div style="display:flex; flex-direction:column; gap:6px;">
                                                <span style="color:green; font-weight:600;">
