...
$orderAccount = OrderAccount::find()->where(['id' => $order_id])->one();
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <?php  foreach ($warehouse as $model) { $i = 1; $allCount = 0; $allSumm = 0;?>
                                ...
                                <?php  foreach ($warehouses = ProductAccount::find()
                                                ->andWhere(['brand_id' => $model->brand->id])
                                                ->andWhere(['order_account_id' => $order_id])->all() as $model1) { ?>
                                    <tr >
                                        ...
                                        <?php if($model1->type_sklad_id ==1){ ?> 
                                            <td style="background-color:#67a38569"><b > <?= $model1->getTypeView($model1->type ==1) ?></b></td>
                                        <?php }else{?>
                                            <td> <b ><?= $model1->getTypeView($model1->type ==1) ?></b></td>
                                        <?php }?>
